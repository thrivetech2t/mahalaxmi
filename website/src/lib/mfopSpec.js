// MFOP Specification v1.0 — structured content for peer review page
// Each section: { id, title, level (1=h2, 2=h3), content, blocks? }
...
Comments and questions should be directed to the author at Ami.nunez@mahalaxmi.ai. The current draft and discussion threads are maintained at https://mahalaxmi.ai/mfop/draft. Discussion threads are at https://mahalaxmi.ai/mfop/discuss.`,
  },
  {
    ...
    level: 1,
    content: `Copyright © 2026 ThriveTech Services LLC. All rights reserved. Permission is granted to copy, distribute, and use this document in any medium without fee, provided that the author attribution, document title, and this copyright notice are preserved in all copies and derivative works.`,
  },
  {
    id: 'abstract',
    title: 'Abstract',
...
MFOP is designed to operate in three simultaneous deployment configurations: private enterprise meshes where nodes are owned and operated by a single organization, managed cloud pools operated by the platform provider, and open community marketplaces where any node operator may contribute compute in exchange for economic settlement. The protocol is agnostic to the underlying AI model provider and is designed to evolve with the AI safety and compliance landscape.`,
  },
  {
    ...
    level: 1,
    content: `The growth of large language model (LLM) deployments across enterprise environments has created a need for a coordination layer that can span heterogeneous compute infrastructure while satisfying compliance, billing, and safety requirements that vary by jurisdiction and industry.
...
This specification defines the wire protocol, data formats, cryptographic mechanisms, and behavioral requirements for all components of a conforming MFOP federation.`,
  },
  {
    ...
    title: '2. Terminology',
    level: 1,
    content: `The key words "MUST", "MUST NOT", "REQUIRED", "SHALL", "SHALL NOT", "SHOULD", "SHOULD NOT", "RECOMMENDED", "NOT RECOMMENDED", "MAY", and "OPTIONAL" in this document are to be interpreted as described in BCP 14 [RFC2119] [RFC8174].
...
**NeMo Guardrails** — The NVIDIA NeMo safety framework used by MFOP nodes for AI safety policy validation and output filtering.`,
  ...
  {
    id: 'section-3',
    title: '3. Node Identity and Registration',
    level: 1,
    content: `Each node in a MFOP federation is identified by a stable, globally unique node identifier (node_id). The node_id is a 128-bit UUID (version 4) assigned at registration time and persists across node restarts and software upgrades.
...
Nodes MUST send a heartbeat to POST /v1/federation/nodes/{id}/heartbeat at least once every 60 seconds. A node that misses three consecutive heartbeat windows is marked INACTIVE and excluded from routing. Nodes may deregister voluntarily via DELETE /v1/federation/nodes/{id}.`,
  ...
  {
    id: 'section-4',
    title: '4. Capability Advertisement',
    level: 1,
    content: `A node's capability advertisement declares the AI models available on the node, the hardware characteristics relevant to job routing, and the compliance certifications held by the node operator.
...
Nodes MUST update their capability advertisement via PUT /v1/federation/nodes/{id}/capabilities whenever their available models or hardware configuration changes. The federation propagates updated capability advertisements to the routing layer within 30 seconds.`,
  ...
  {
    id: 'section-5',
    title: '5. Compliance-Zone-Aware Job Routing',
    level: 1,
    content: `MFOP routes each job to a node that satisfies the job's compliance zone requirements. Compliance zone satisfaction is a hard constraint: a job MUST NOT be routed to a node that is not certified for the job's compliance zone.
...
Affinity rules affect only the affinity_score component; compliance zone certification and capacity remain hard constraints.`,
  ...
  {
    id: 'section-6',
    title: '6. Semantic Input Partitioning',
    level: 1,
    content: `For jobs whose input exceeds a single node's max_context_tokens, MFOP provides a semantic partitioning mechanism that splits the input into coherent sub-jobs, routes each sub-job independently, and aggregates the results.
...
Once all sub-jobs complete, the federation's aggregation layer assembles the results in sequence-number order. For sliding_window partitions, the aggregator de-duplicates content in the overlap regions using a longest-common-subsequence merge. The assembled result is returned to the submitter as a single JobResult with an array of sub_job_receipts.`,
  ...
  {
    id: 'section-7',
    title: '7. Cryptographically Signed Billing Receipts',
    level: 1,
    content: `Every completed job execution produces a BillingReceipt signed by the executing node. Signed receipts are the authoritative record for economic settlement and dispute resolution.
...
The federation stores all receipts for a minimum of 7 years to support compliance audit requirements. Submitters may retrieve their receipts via GET /v1/federation/receipts. Node operators may retrieve receipts for jobs they executed via GET /v1/federation/nodes/{id}/receipts.`,
  ...
  {
    id: 'section-8',
    title: '8. Configurable Economic Settlement',
    level: 1,
    content: `MFOP separates billing (the accumulation of signed receipts) from settlement (the financial transfer of funds). Settlement is configurable and may occur on different schedules for different participant types.
...
Node operators are paid out via Stripe Connect at the end of each settlement period, provided their accumulated earnings exceed the minimum_payout_usd threshold. Operators who do not meet the threshold roll their earnings forward to the next period.`,
  },
  {
    ...
    level: 1,
    content: `MFOP implements a three-layer security model: transport security, AI safety policy validation, and execution sandbox isolation.
...
All job routing decisions, receipt signatures, and settlement events are written to an append-only audit log. The audit log is cryptographically chained using SHA-256 hashes (each entry includes the hash of the previous entry). The audit log may not be modified; only append operations are permitted.`,
  ...
  {
    id: 'section-10',
    title: '10. Wire Protocol',
    level: 1,
    content: `MFOP uses JSON over HTTPS for all API communication. WebSocket connections are supported for streaming job output (see Section 10.2).
...
Job submission requests SHOULD include an Idempotency-Key header (UUID). If a request with the same Idempotency-Key is received within the idempotency window (24 hours), the federation returns the original response without re-executing the job. This protects against duplicate submissions caused by network retries.`,
  ...
  {
    id: 'appendix-a',
    title: 'Appendix A. REST API Reference',
    level: 1,
    content: `This appendix lists the MFOP REST API endpoints. All endpoints require an X-Channel-API-Key header unless noted otherwise. Base path: /v1/federation`,
    blocks: [
      {
        type: 'table',
        headers: ['Method + Path', 'Name', 'Description'],
        rows: [
  ...
  {
    id: 'appendix-b',
    title: 'Appendix B. Compliance Zone Policy Requirements',
    level: 1,
    content: `Each compliance zone requires specific NeMo Guardrails policy capabilities beyond the baseline. The following table summarizes the minimum required rails per zone.`,
    blocks: [
      {
        type: 'table',
        headers: ['Zone', 'Required Rails Beyond Baseline'],
        rows: [
  ...
  },
  {
    ...
    level: 1,
    content: `The author wishes to acknowledge the NVIDIA NeMo team for the NeMo Guardrails and NemoClaw OpenShell platforms, which provide the foundational security infrastructure referenced in this specification. The MFOP security model is designed to evolve with these platforms as they mature.
