// MFOP Spezifikation v1.0 — strukturierter Inhalt für die Peer-Review-Seite
// Jeder Abschnitt: { id, title, level (1=h2, 2=h3), content, blocks? }
...
export const mfopMeta = {
  ...
  shortTitle: 'MFOP',
  version: '1.0',
  status: 'Entwurf zur Begutachtung',
  date: 'März 2026',
...
Diese Spezifikation definiert das Leitungsprotokoll, Datenformate, kryptographische Mechanismen und Verhaltensanforderungen für alle Komponenten einer konformen MFOP-Föderation.`,
  },
  {
    ...
    level: 1,
    content: `Die Schlüsselwörter "MUSS", "DARF NICHT", "ERFORDERLICH", "SOLL", "SOLL NICHT", "SOLLTE", "SOLLTE NICHT", "EMPFOHLEN", "NICHT EMPFOHLEN", "KANN" und "OPTIONAL" in diesem Dokument sind gemäß BCP 14 [RFC2119] [RFC8174] zu interpretieren.
...
**NeMo Guardrails** — Das NVIDIA NeMo Sicherheits-Framework, das von MFOP-Knoten für die Validierung von KI-Sicherheitsrichtlinien und Ausgabefilterung verwendet wird.`,
  ...
  {
    id: 'section-3',
    title: '3. Knotenidentität und Registrierung',
    level: 1,
    content: `Jeder Knoten in einer MFOP-Föderation wird durch einen stabilen, global eindeutigen Knotenbezeichner (node_id) identifiziert. Die node_id ist ein 128-Bit-UUID (Version 4), der bei der Registrierung vergeben wird und über Knotenneustart und Software-Upgrades hinaus bestehen bleibt.
...
Knoten MÜSSEN mindestens einmal alle 60 Sekunden einen Heartbeat an POST /v1/federation/nodes/{id}/heartbeat senden. Ein Knoten, der drei aufeinanderfolgende Heartbeat-Fenster verpasst, wird als INAKTIV markiert und vom Routing ausgeschlossen. Knoten können sich freiwillig über DELETE /v1/federation/nodes/{id} abmelden.`,
  ...
  {
    id: 'section-4',
    title: '4. Fähigkeitswerbung',
    level: 1,
    content: `Die Fähigkeitswerbung eines Knotens deklariert die auf dem Knoten verfügbaren KI-Modelle, die für das Job-Routing relevanten Hardware-Eigenschaften und die vom Knotenbetreiber gehaltenen Compliance-Zertifizierungen.
...
Knoten MÜSSEN ihre Fähigkeitswerbung über PUT /v1/federation/nodes/{id}/capabilities aktualisieren, wann immer sich ihre verfügbaren Modelle oder Hardware-Konfiguration ändert. Die Föderation überträgt aktualisierte Fähigkeitswerbungen innerhalb von 30 Sekunden an die Routing-Schicht.`,
  ...
  {
    id: 'section-5',
    title: '5. Compliance-Zone-gerechtes Job-Routing',
    level: 1,
    content: `MFOP leitet jeden Job an einen Knoten weiter, der die Compliance-Zone-Anforderungen des Jobs erfüllt. Die Erfüllung der Compliance-Zone ist eine harte Bedingung: Ein Job DARF NICHT an einen Knoten weitergeleitet werden, der nicht für die Compliance-Zone des Jobs zertifiziert ist.
...
Affinitätsregeln beeinflussen nur die affinity_score-Komponente; Compliance-Zone-Zertifizierung und Kapazität bleiben harte Bedingungen.`,
  ...
  {
    id: 'section-6',
    title: '6. Semantische Eingabeaufteilung',
    level: 1,
    content: `Für Jobs, deren Eingabe die max_context_tokens eines einzelnen Knotens überschreitet, bietet MFOP einen semantischen Aufteilungsmechanismus, der die Eingabe in kohärente Teilaufgaben aufteilt, jeden Teilauftrag unabhängig weiterleitet und die Ergebnisse zusammenführt.
...
Sobald alle Teilaufträge abgeschlossen sind, stellt die Zusammenführungsschicht der Föderation die Ergebnisse in der Reihenfolge der Sequenznummern zusammen. Für sliding_window-Partitionen dedupliziert der Aggregator Inhalte in den Überlappungsbereichen mittels einer Longest-Common-Subsequence-Zusammenführung. Das zusammengestellte Ergebnis wird dem Einreicher als einzelnes JobResult mit einem Array von sub_job_receipts zurückgegeben.`,
  ...
  {
    id: 'section-7',
    title: '7. Kryptographisch signierte Abrechnungsbelege',
    level: 1,
    content: `Jede abgeschlossene Job-Ausführung erzeugt einen BillingReceipt, der vom ausführenden Knoten signiert wird. Signierte Belege sind der maßgebliche Datensatz für wirtschaftliche Abrechnung und Streitbeilegung.
...
Die Föderation speichert alle Belege mindestens 7 Jahre lang, um Compliance-Audit-Anforderungen zu unterstützen. Einreicher können ihre Belege über GET /v1/federation/receipts abrufen. Knotenbetreiber können Belege für von ihnen ausgeführte Jobs über GET /v1/federation/nodes/{id}/receipts abrufen.`,
  ...
  {
    id: 'section-8',
    title: '8. Konfigurierbare wirtschaftliche Abrechnung',
    level: 1,
    content: `MFOP trennt Abrechnung (die Anhäufung signierter Belege) von Abwicklung (die finanzielle Überweisung von Geldern). Die Abwicklung ist konfigurierbar und kann für verschiedene Teilnehmertypen nach unterschiedlichen Zeitplänen erfolgen.
...
Knotenbetreiber werden am Ende jedes Abrechnungszeitraums über Stripe Connect ausgezahlt, sofern ihre angesammelten Einnahmen den Schwellenwert minimum_payout_usd überschreiten. Betreiber, die den Schwellenwert nicht erreichen, übertragen ihre Einnahmen in den nächsten Zeitraum.`,
  ...
  {
    id: 'section-9',
    title: '9. Sicherheitsmodell',
    level: 1,
    content: `MFOP implementiert ein dreischichtiges Sicherheitsmodell: Transportsicherheit, KI-Sicherheitsrichtlinienvalidierung und Ausführungs-Sandbox-Isolation.
...
Alle Job-Routing-Entscheidungen, Beleguнterzeichnungen und Abwicklungsereignisse werden in ein append-only-Prüfungsprotokoll geschrieben. Das Prüfungsprotokoll ist kryptographisch mit SHA-256-Hashes verkettet (jeder Eintrag enthält den Hash des vorherigen Eintrags). Das Prüfungsprotokoll darf nicht geändert werden; nur Anhängeoperationen sind zulässig.`,
  ...
  {
    id: 'section-10',
    title: '10. Leitungsprotokoll',
    level: 1,
    content: `MFOP verwendet JSON über HTTPS für die gesamte API-Kommunikation. WebSocket-Verbindungen werden für das Streaming von Job-Ausgaben unterstützt (siehe Abschnitt 10.2).
...
Job-Übermittlungsanfragen SOLLTEN einen Idempotency-Key-Header (UUID) enthalten. Wenn eine Anfrage mit demselben Idempotency-Key innerhalb des Idempotenzfensters (24 Stunden) empfangen wird, gibt die Föderation die ursprüngliche Antwort zurück, ohne den Job erneut auszuführen. Dies schützt vor doppelten Übermittlungen, die durch Netzwerk-Wiederholungsversuche verursacht werden.`,
  ...
  {
    id: 'appendix-a',
    title: 'Anhang A. REST-API-Referenz',
    level: 1,
    content: `Dieser Anhang listet die MFOP-REST-API-Endpunkte auf. Alle Endpunkte erfordern einen X-Channel-API-Key-Header, sofern nicht anders angegeben. Basispfad: /v1/federation`,
    blocks: [
      {
        type: 'table',
        headers: ['Methode + Pfad', 'Name', 'Beschreibung'],
        rows: [
  ...
  {
    id: 'appendix-b',
    title: 'Anhang B. Compliance-Zone-Richtlinienanforderungen',
    level: 1,
    content: `Jede Compliance-Zone erfordert spezifische NeMo Guardrails-Richtlinienfähigkeiten über den Grundsatz hinaus. Die folgende Tabelle fasst die minimal erforderlichen Schienen pro Zone zusammen.`,
    blocks: [
      {
        type: 'table',
        headers: ['Zone', 'Erforderliche Schienen über den Grundsatz hinaus'],
        rows: [
