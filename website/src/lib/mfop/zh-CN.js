// MFOP 规范 v1.0 — 结构化内容，用于同行评审页面
// 每个章节：{ id, title, level (1=h2, 2=h3), content, blocks? }
// blocks: [{ type: 'table', headers: [...], rows: [[...]] } | { type: 'note', content }]

export const mfopMeta = {
  title: 'Mahalaxmi 联邦与编排协议',
  shortTitle: 'MFOP',
  version: '1.0',
  status: '同行评审草稿',
  date: '2026年3月',
...
export const mfopSections = [
  {
    ...
    title: '本备忘录状态',
    level: 1,
    content: `本文档是 Mahalaxmi 联邦与编排协议（MFOP）规范 1.0 版本的预发布草稿，面向同行评审发布，征集意见。本文档描述了一种协议，用于在异构计算节点上实现联邦式分布式 AI 编排，支持合规区域感知路由、密码学签名计费收据以及可配置的经济结算。
...
意见和问题请发送至作者邮箱 Ami.nunez@mahalaxmi.ai。当前草稿及讨论线程维护于 https://mahalaxmi.ai/mfop/draft，讨论区地址为 https://mahalaxmi.ai/mfop/discuss。`,
  },
  {
    ...
    title: '版权声明',
    level: 1,
    content: `版权所有 © 2026 ThriveTech Services LLC。保留所有权利。在任何介质中，允许免费复制、分发和使用本文档，但须在所有副本及衍生作品中保留作者署名、文档标题及本版权声明。`,
  },
  {
    id: 'abstract',
    title: '摘要',
    level: 1,
    content: `本文档定义了 Mahalaxmi 联邦与编排协议（MFOP），这是一种用于在异构计算节点分布式网络中协调并行 AI 代理执行的协议。MFOP 规定了节点身份与注册、能力通告、合规区域感知的作业路由、语义输入分区、密码学签名计费收据、可配置的经济结算，以及使用 AI 安全策略验证和执行沙箱隔离的分层安全模型。
...
MFOP 被设计为同时支持三种部署配置：由单一组织拥有并运营节点的私有企业网格、由平台提供商运营的托管云资源池，以及任何节点运营商均可贡献算力并获得经济结算的开放社区市场。该协议对底层 AI 模型提供商保持中立，并设计为随 AI 安全与合规领域的发展持续演进。`,
  },
  {
    ...
    title: '1. 引言',
    level: 1,
    content: `大型语言模型（LLM）在企业环境中的大规模部署，产生了对协调层的需求——该协调层需要能够跨越异构计算基础设施，同时满足因司法管辖区和行业不同而各异的合规、计费和安全要求。
...
本规范定义了符合 MFOP 标准的联邦中所有组件的线路协议、数据格式、密码机制和行为要求。`,
  },
  {
    ...
    title: '2. 术语',
    level: 1,
    content: `本文档中的关键词"必须"（MUST）、"不得"（MUST NOT）、"必需"（REQUIRED）、"应当"（SHALL）、"不应当"（SHALL NOT）、"应该"（SHOULD）、"不应该"（SHOULD NOT）、"推荐"（RECOMMENDED）、"不推荐"（NOT RECOMMENDED）、"可以"（MAY）和"可选"（OPTIONAL），须按照 BCP 14 [RFC2119] [RFC8174] 中的描述加以解释。
...
**NeMo Guardrails** — NVIDIA NeMo 安全框架，由 MFOP 节点用于 AI 安全策略验证和输出过滤。`,
  },
  {
    ...
    title: '3. 节点身份与注册',
    level: 1,
    content: `MFOP 联邦中的每个节点由一个稳定、全球唯一的节点标识符（node_id）标识。node_id 是在注册时分配的 128 位 UUID（第 4 版），在节点重启和软件升级后保持不变。
...
节点**必须**至少每 60 秒向 POST /v1/federation/nodes/{id}/heartbeat 发送一次心跳。连续错过三个心跳窗口的节点将被标记为 INACTIVE 并从路由中排除。节点可通过 DELETE /v1/federation/nodes/{id} 主动注销。`,
  },
  {
    ...
    title: '4. 能力通告',
    level: 1,
    content: `节点的能力通告声明了节点上可用的 AI 模型、与作业路由相关的硬件特性，以及节点运营商持有的合规认证。
...
当节点的可用模型或硬件配置发生变化时，节点**必须**通过 PUT /v1/federation/nodes/{id}/capabilities 更新其能力通告。联邦在 30 秒内将更新后的能力通告传播至路由层。`,
  },
  {
    ...
    title: '5. 合规区域感知的作业路由',
    level: 1,
    content: `MFOP 将每个作业路由至满足其合规区域要求的节点。合规区域满足条件是硬性约束：作业**不得**被路由至未经认证支持该合规区域的节点。
...
亲和性规则仅影响 affinity_score 分量；合规区域认证和容量仍为硬性约束。`,
  },
  {
    ...
    title: '6. 语义输入分区',
    level: 1,
    content: `对于输入超过单个节点 max_context_tokens 的作业，MFOP 提供语义分区机制，将输入拆分为语义连贯的子作业，独立路由每个子作业，并聚合结果。
...
所有子作业完成后，联邦的聚合层按序号顺序组装结果。对于 sliding_window 分区，聚合器使用最长公共子序列合并对重叠区域的内容进行去重。组装后的结果作为包含 sub_job_receipts 数组的单个 JobResult 返回给提交方。`,
  },
  {
    ...
    title: '7. 密码学签名计费收据',
    level: 1,
    content: `每次完成的作业执行均会产生一份由执行节点签名的 BillingReceipt。签名收据是经济结算和争议解决的权威记录。
...
联邦将所有收据至少保存 7 年，以支持合规审计要求。提交方可通过 GET /v1/federation/receipts 检索其收据。节点运营商可通过 GET /v1/federation/nodes/{id}/receipts 检索其执行作业的收据。`,
  },
  {
    ...
    title: '8. 可配置的经济结算',
    level: 1,
    content: `MFOP 将计费（签名收据的累积）与结算（资金的财务转账）分离。结算是可配置的，可按照不同参与者类型的不同时间表进行。
...
节点运营商在每个结算周期结束时通过 Stripe Connect 获得付款，前提是其累计收益超过 minimum_payout_usd 阈值。未达到阈值的运营商将其收益结转至下一周期。`,
  },
  {
    ...
    title: '9. 安全模型',
    level: 1,
    content: `MFOP 实现三层安全模型：传输安全、AI 安全策略验证和执行沙箱隔离。
...
所有作业路由决策、收据签名和结算事件均写入仅追加审计日志。审计日志使用 SHA-256 哈希进行密码学链式记录（每个条目包含前一条目的哈希值）。审计日志不得修改，仅允许追加操作。`,
  },
  {
    ...
    title: '10. 线路协议',
    level: 1,
    content: `MFOP 对所有 API 通信使用基于 HTTPS 的 JSON。WebSocket 连接支持流式作业输出（见第 10.2 节）。
...
作业提交请求**应该**包含 Idempotency-Key 头部（UUID）。若在幂等性窗口期（24 小时）内收到具有相同 Idempotency-Key 的请求，联邦将返回原始响应而不重新执行作业。这可防止由网络重试导致的重复提交。`,
  ...
  {
    id: 'appendix-a',
    title: '附录 A. REST API 参考',
    level: 1,
    content: `本附录列出 MFOP REST API 端点。除另有说明外，所有端点均需要 X-Channel-API-Key 头部。基础路径：/v1/federation`,
    blocks: [
      {
        type: 'table',
        headers: ['方法 + 路径', '名称', '描述'],
  ...
  },
  {
    ...
    title: '附录 B. 合规区域策略要求',
    level: 1,
    content: `每个合规区域在基线之外要求特定的 NeMo Guardrails 策略能力。下表汇总了每个区域所需的最低护栏配置。`,
    blocks: [
      {
        type: 'table',
        headers: ['区域', '基线之外所需护栏'],
  ...
  },
  {
    ...
    title: '致谢',
    level: 1,
    content: `作者谨向 NVIDIA NeMo 团队致谢，感谢其开发的 NeMo Guardrails 和 NemoClaw OpenShell 平台，这些平台为本规范所引用的基础安全基础设施提供了支撑。MFOP 安全模型的设计旨在随这些平台的成熟持续演进。
