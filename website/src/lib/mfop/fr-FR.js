// MFOP Specification v1.0 — structured content for peer review page (French / fr-FR)
...
export const mfopMeta = {
  title: "Protocole de Fédération et d'Orchestration Mahalaxmi",
  shortTitle: 'MFOP',
  version: '1.0',
  status: "Brouillon soumis à l'examen par les pairs",
  date: 'Mars 2026',
...
Cette spécification définit le protocole filaire, les formats de données, les mécanismes cryptographiques, et les exigences comportementales pour tous les composants d'une fédération MFOP conforme.`,
  ...
  {
    id: 'section-2',
    title: '2. Terminologie',
    level: 1,
    content: `Les mots-clés « DOIT », « NE DOIT PAS », « REQUIS », « DEVRA », « NE DEVRA PAS », « DEVRAIT », « NE DEVRAIT PAS », « RECOMMANDÉ », « NON RECOMMANDÉ », « PEUT » et « OPTIONNEL » dans ce document doivent être interprétés comme décrit dans BCP 14 [RFC2119] [RFC8174].
...
**NeMo Guardrails** — Le cadre de sécurité NVIDIA NeMo utilisé par les nœuds MFOP pour la validation des politiques de sécurité IA et le filtrage des sorties.`,
  ...
  {
    id: 'section-3',
    title: "3. Identité et enregistrement des nœuds",
    level: 1,
    content: `Chaque nœud d'une fédération MFOP est identifié par un identifiant de nœud stable et globalement unique (node_id). Le node_id est un UUID 128 bits (version 4) attribué au moment de l'enregistrement et persiste au travers des redémarrages et mises à jour logicielles du nœud.
...
Les nœuds DOIVENT envoyer un battement de cœur à POST /v1/federation/nodes/{id}/heartbeat au moins une fois toutes les 60 secondes. Un nœud qui manque trois fenêtres de battement de cœur consécutives est marqué INACTIF et exclu du routage. Les nœuds peuvent se désenregistrer volontairement via DELETE /v1/federation/nodes/{id}.`,
  ...
  {
    id: 'section-4',
    title: '4. Publicité des capacités',
    level: 1,
    content: `La publicité des capacités d'un nœud déclare les modèles IA disponibles sur le nœud, les caractéristiques matérielles pertinentes pour le routage des tâches, et les certifications de conformité détenues par l'opérateur du nœud.
...
Les nœuds DOIVENT mettre à jour leur publicité des capacités via PUT /v1/federation/nodes/{id}/capabilities chaque fois que leurs modèles disponibles ou leur configuration matérielle changent. La fédération propage les publicités de capacités mises à jour vers la couche de routage en moins de 30 secondes.`,
  ...
  {
    id: 'section-5',
    title: '5. Routage des tâches tenant compte des zones de conformité',
    level: 1,
    content: `MFOP achemine chaque tâche vers un nœud qui satisfait les exigences de zone de conformité de la tâche. La satisfaction de la zone de conformité est une contrainte stricte : une tâche NE DOIT PAS être acheminée vers un nœud qui n'est pas certifié pour la zone de conformité de la tâche.
...
Les règles d'affinité n'affectent que la composante affinity_score ; la certification de zone de conformité et la capacité restent des contraintes strictes.`,
  ...
  {
    id: 'section-6',
    title: '6. Partitionnement sémantique des entrées',
    level: 1,
    content: `Pour les tâches dont l'entrée dépasse le max_context_tokens d'un seul nœud, MFOP fournit un mécanisme de partitionnement sémantique qui divise l'entrée en sous-tâches cohérentes, achemine chaque sous-tâche indépendamment, et agrège les résultats.
...
Une fois toutes les sous-tâches terminées, la couche d'agrégation de la fédération assemble les résultats dans l'ordre des numéros de séquence. Pour les partitions sliding_window, l'agrégateur déduplique le contenu dans les régions de chevauchement en utilisant une fusion par plus longue sous-séquence commune. Le résultat assemblé est retourné au soumissionnaire sous la forme d'un JobResult unique avec un tableau de sub_job_receipts.`,
  ...
  {
    id: 'section-7',
    title: '7. Reçus de facturation signés cryptographiquement',
    level: 1,
    content: `Chaque exécution de tâche complétée produit un BillingReceipt signé par le nœud exécutant. Les reçus signés constituent l'enregistrement faisant autorité pour le règlement économique et la résolution des litiges.
...
La fédération stocke tous les reçus pendant un minimum de 7 ans pour satisfaire les exigences d'audit de conformité. Les soumissionnaires peuvent récupérer leurs reçus via GET /v1/federation/receipts. Les opérateurs de nœuds peuvent récupérer les reçus des tâches qu'ils ont exécutées via GET /v1/federation/nodes/{id}/receipts.`,
  ...
  {
    id: 'section-8',
    title: '8. Règlement économique configurable',
    level: 1,
    content: `MFOP sépare la facturation (l'accumulation des reçus signés) du règlement (le transfert financier des fonds). Le règlement est configurable et peut se produire selon des calendriers différents pour différents types de participants.
...
Les opérateurs de nœuds sont payés via Stripe Connect à la fin de chaque période de règlement, à condition que leurs gains accumulés dépassent le seuil minimum_payout_usd. Les opérateurs qui n'atteignent pas le seuil reportent leurs gains à la période suivante.`,
  ...
  {
    id: 'section-9',
    title: '9. Modèle de sécurité',
    level: 1,
    content: `MFOP met en œuvre un modèle de sécurité à trois couches : la sécurité du transport, la validation des politiques de sécurité IA, et l'isolation des bacs à sable d'exécution.
...
Toutes les décisions de routage des tâches, les signatures de reçus, et les événements de règlement sont écrits dans un journal d'audit en mode ajout uniquement. Le journal d'audit est enchaîné cryptographiquement à l'aide de hachages SHA-256 (chaque entrée inclut le hachage de l'entrée précédente). Le journal d'audit ne peut pas être modifié ; seules les opérations d'ajout sont autorisées.`,
  ...
  {
    id: 'section-10',
    title: '10. Protocole filaire',
    level: 1,
    content: `MFOP utilise JSON sur HTTPS pour toutes les communications API. Les connexions WebSocket sont supportées pour la diffusion en continu des sorties de tâches (voir Section 10.2).
...
Les demandes de soumission de tâches DEVRAIENT inclure un en-tête Idempotency-Key (UUID). Si une demande avec le même Idempotency-Key est reçue dans la fenêtre d'idempotence (24 heures), la fédération retourne la réponse originale sans ré-exécuter la tâche. Cela protège contre les soumissions en double causées par les nouvelles tentatives réseau.`,
  ...
  {
    id: 'appendix-a',
    title: "Annexe A. Référence de l'API REST",
    level: 1,
    content: `Cette annexe liste les points de terminaison de l'API REST MFOP. Tous les points de terminaison requièrent un en-tête X-Channel-API-Key sauf indication contraire. Chemin de base : /v1/federation`,
    blocks: [
      {
        type: 'table',
        headers: ['Méthode + Chemin', 'Nom', 'Description'],
        rows: [
  ...
  {
    id: 'appendix-b',
    title: 'Annexe B. Exigences de politique par zone de conformité',
    level: 1,
    content: `Chaque zone de conformité requiert des capacités de politique NeMo Guardrails spécifiques au-delà de la base. Le tableau suivant résume les rails minimaux requis par zone.`,
    blocks: [
      {
        type: 'table',
        headers: ['Zone', 'Rails requis au-delà de la base'],
        rows: [
