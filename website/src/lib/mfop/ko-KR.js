...
export const mfopMeta = {
  title: 'Mahalaxmi 페더레이션 및 오케스트레이션 프로토콜',
  shortTitle: 'MFOP',
  version: '1.0',
  status: '동료 검토를 위한 초안',
  date: '2026년 3월',
...
export const mfopSections = [
  {
    ...
    title: '이 메모의 상태',
    level: 1,
    content: `이 문서는 Mahalaxmi 페더레이션 및 오케스트레이션 프로토콜(MFOP) 사양 버전 1.0의 출판 전 초안입니다. 동료 검토 및 의견 수렴을 위해 배포됩니다. 이 문서는 컴플라이언스 구역 인식 라우팅, 암호학적으로 서명된 청구 영수증, 구성 가능한 경제적 정산을 갖춘 이기종 컴퓨트 노드 전반에 걸친 페더레이션 분산 AI 오케스트레이션 프로토콜을 설명합니다.
...
의견과 질문은 Ami.nunez@mahalaxmi.ai의 저자에게 직접 보내주십시오. 현재 초안과 토론 스레드는 https://mahalaxmi.ai/mfop/draft 에서 유지 관리됩니다. 토론 스레드는 https://mahalaxmi.ai/mfop/discuss 에 있습니다.`,
  },
  {
    ...
    title: '저작권 고지',
    level: 1,
    content: `Copyright © 2026 ThriveTech Services LLC. All rights reserved. 저자 표시, 문서 제목 및 이 저작권 고지가 모든 사본 및 파생 저작물에 보존되는 조건 하에, 어떠한 매체에서도 이 문서를 무료로 복사, 배포 및 사용할 수 있는 권한이 부여됩니다.`,
  },
  {
    id: 'abstract',
    title: '초록',
    level: 1,
    content: `이 문서는 이기종 컴퓨트 노드의 분산 네트워크 전반에서 병렬 AI 에이전트 실행을 조율하기 위한 프로토콜인 Mahalaxmi 페더레이션 및 오케스트레이션 프로토콜(MFOP)을 정의합니다. MFOP는 노드 신원 및 등록, 능력 광고, 컴플라이언스 구역 인식 작업 라우팅, 의미론적 입력 분할, 암호학적으로 서명된 청구 영수증, 구성 가능한 경제적 정산, 그리고 AI 안전 정책 검증 및 실행 샌드박스 격리를 사용하는 계층화된 보안 모델을 명시합니다.
...
MFOP는 세 가지 동시 배포 구성으로 운영되도록 설계되었습니다: 단일 조직이 노드를 소유하고 운영하는 프라이빗 엔터프라이즈 메시, 플랫폼 제공업체가 운영하는 관리형 클라우드 풀, 그리고 모든 노드 운영자가 경제적 정산의 대가로 컴퓨트를 기여할 수 있는 오픈 커뮤니티 마켓플레이스. 이 프로토콜은 기반 AI 모델 제공업체에 구애받지 않으며, AI 안전 및 컴플라이언스 환경의 발전에 따라 진화하도록 설계되었습니다.`,
  },
  {
    ...
    title: '1. 소개',
    level: 1,
    content: `엔터프라이즈 환경 전반에서 대규모 언어 모델(LLM) 배포의 성장으로 인해, 관할권 및 산업에 따라 다양한 컴플라이언스, 청구, 안전 요구사항을 충족하면서 이기종 컴퓨트 인프라에 걸쳐 작동할 수 있는 조율 레이어의 필요성이 생겨났습니다.
...
이 사양은 MFOP 페더레이션의 모든 구성 요소에 대한 와이어 프로토콜, 데이터 형식, 암호화 메커니즘, 그리고 동작 요구사항을 정의합니다.`,
  },
  {
    ...
    title: '2. 용어',
    level: 1,
    content: `이 문서에서 "MUST", "MUST NOT", "REQUIRED", "SHALL", "SHALL NOT", "SHOULD", "SHOULD NOT", "RECOMMENDED", "NOT RECOMMENDED", "MAY", "OPTIONAL"이라는 핵심 용어는 BCP 14 [RFC2119] [RFC8174]에 설명된 바에 따라 해석되어야 합니다. 한국어로는 각각 "반드시 ~해야 한다", "~해서는 안 된다", "필수", "~해야 한다", "~해서는 안 된다", "~하는 것이 좋다", "~하지 않는 것이 좋다", "권장됨", "권장되지 않음", "~해도 된다", "선택 사항"으로 해석됩니다.
...
**NeMo Guardrails** — MFOP 노드가 AI 안전 정책 검증 및 출력 필터링에 사용하는 NVIDIA NeMo 안전 프레임워크.`,
  },
  {
    ...
    title: '3. 노드 신원 및 등록',
    level: 1,
    content: `MFOP 페더레이션의 각 노드는 안정적이고 전역적으로 고유한 노드 식별자(node_id)로 식별됩니다. node_id는 등록 시 할당되는 128비트 UUID(버전 4)이며, 노드 재시작 및 소프트웨어 업그레이드 전반에 걸쳐 유지됩니다.
...
노드는 반드시 최소 60초마다 POST /v1/federation/nodes/{id}/heartbeat에 하트비트를 전송해야 합니다. 세 번의 연속 하트비트 창을 놓친 노드는 INACTIVE로 표시되어 라우팅에서 제외됩니다. 노드는 DELETE /v1/federation/nodes/{id}를 통해 자발적으로 등록을 해제할 수 있습니다.`,
  },
  {
    ...
    title: '4. 능력 광고',
    level: 1,
    content: `노드의 능력 광고는 해당 노드에서 사용 가능한 AI 모델, 작업 라우팅과 관련된 하드웨어 특성, 그리고 노드 운영자가 보유한 컴플라이언스 인증을 선언합니다.
...
노드는 사용 가능한 모델이나 하드웨어 구성이 변경될 때마다 반드시 PUT /v1/federation/nodes/{id}/capabilities를 통해 능력 광고를 업데이트해야 합니다. 페더레이션은 업데이트된 능력 광고를 30초 이내에 라우팅 레이어에 전파합니다.`,
  ...
  {
    id: 'section-5',
    title: '5. 컴플라이언스 구역 인식 작업 라우팅',
    level: 1,
    content: `MFOP는 각 작업을 해당 작업의 컴플라이언스 구역 요구사항을 충족하는 노드로 라우팅합니다. 컴플라이언스 구역 충족은 하드 제약 조건입니다: 작업은 해당 작업의 컴플라이언스 구역에 대해 인증받지 않은 노드로 라우팅되어서는 안 됩니다.
...
어피니티 규칙은 affinity_score 구성 요소에만 영향을 미치며, 컴플라이언스 구역 인증 및 용량은 하드 제약 조건으로 유지됩니다.`,
  },
  {
    ...
    title: '6. 의미론적 입력 분할',
    level: 1,
    content: `입력이 단일 노드의 max_context_tokens을 초과하는 작업의 경우, MFOP는 입력을 일관된 서브 작업으로 분할하고, 각 서브 작업을 독립적으로 라우팅하며, 결과를 집계하는 의미론적 분할 메커니즘을 제공합니다.
...
모든 서브 작업이 완료되면, 페더레이션의 집계 레이어는 일련 번호 순으로 결과를 조합합니다. sliding_window 분할의 경우, 집계자는 최장 공통 부분 수열 병합을 사용하여 겹침 영역의 콘텐츠를 중복 제거합니다. 조합된 결과는 sub_job_receipts 배열과 함께 단일 JobResult로 제출자에게 반환됩니다.`,
  ...
  {
    id: 'section-7',
    title: '7. 암호학적으로 서명된 청구 영수증',
    level: 1,
    content: `모든 완료된 작업 실행은 실행 노드가 서명한 BillingReceipt를 생성합니다. 서명된 영수증은 경제적 정산 및 분쟁 해결을 위한 권위 있는 기록입니다.
...
페더레이션은 컴플라이언스 감사 요구사항을 지원하기 위해 최소 7년간 모든 영수증을 저장합니다. 제출자는 GET /v1/federation/receipts를 통해 영수증을 조회할 수 있습니다. 노드 운영자는 GET /v1/federation/nodes/{id}/receipts를 통해 자신이 실행한 작업의 영수증을 조회할 수 있습니다.`,
  ...
  {
    id: 'section-8',
    title: '8. 구성 가능한 경제적 정산',
    level: 1,
    content: `MFOP는 청구(서명된 영수증 누적)를 정산(자금의 재정적 이체)과 분리합니다. 정산은 구성 가능하며 참여자 유형에 따라 다른 일정으로 발생할 수 있습니다.
...
노드 운영자는 누적 수익이 minimum_payout_usd 임계값을 초과하는 경우, 각 정산 기간 말에 Stripe Connect를 통해 지급받습니다. 임계값을 충족하지 못하는 운영자는 수익을 다음 기간으로 이월합니다.`,
  },
  {
    ...
    title: '9. 보안 모델',
    level: 1,
    content: `MFOP는 세 가지 레이어의 보안 모델을 구현합니다: 전송 보안, AI 안전 정책 검증, 실행 샌드박스 격리.
...
모든 작업 라우팅 결정, 영수증 서명 및 정산 이벤트는 추가 전용 감사 로그에 기록됩니다. 감사 로그는 SHA-256 해시를 사용하여 암호학적으로 연결됩니다(각 항목에는 이전 항목의 해시가 포함됩니다). 감사 로그는 수정할 수 없으며, 추가 작업만 허용됩니다.`,
  },
  {
    ...
    title: '10. 와이어 프로토콜',
    level: 1,
    content: `MFOP는 모든 API 통신에 HTTPS를 통한 JSON을 사용합니다. WebSocket 연결은 스트리밍 작업 출력을 지원합니다 (섹션 10.2 참조).
...
작업 제출 요청에는 Idempotency-Key 헤더(UUID)를 포함하는 것이 좋습니다. 멱등성 창(24시간) 내에 동일한 Idempotency-Key를 가진 요청이 수신되면, 페더레이션은 작업을 재실행하지 않고 원래 응답을 반환합니다. 이는 네트워크 재시도로 인한 중복 제출을 방지합니다.`,
  ...
  {
    id: 'appendix-a',
    title: '부록 A. REST API 참조',
    level: 1,
    content: `이 부록은 MFOP REST API 엔드포인트를 나열합니다. 별도로 명시되지 않는 한 모든 엔드포인트에는 X-Channel-API-Key 헤더가 필요합니다. 기본 경로: /v1/federation`,
    blocks: [
      {
        type: 'table',
        headers: ['메서드 + 경로', '이름', '설명'],
        rows: [
  ...
  {
    id: 'appendix-b',
    title: '부록 B. 컴플라이언스 구역 정책 요구사항',
    level: 1,
    content: `각 컴플라이언스 구역은 기준을 넘어 특정 NeMo Guardrails 정책 기능을 필요로 합니다. 다음 표는 구역별 최소 필수 레일을 요약합니다.`,
    blocks: [
      {
        type: 'table',
        headers: ['구역', '기준 이외의 필수 레일'],
        rows: [
  ...
  },
  {
    ...
    title: '감사의 말',
    level: 1,
    content: `저자는 이 사양에서 참조된 기반 보안 인프라를 제공하는 NeMo Guardrails 및 NemoClaw OpenShell 플랫폼에 대해 NVIDIA NeMo 팀에 감사드립니다. MFOP 보안 모델은 이러한 플랫폼이 성숙해짐에 따라 함께 발전하도록 설계되었습니다.
