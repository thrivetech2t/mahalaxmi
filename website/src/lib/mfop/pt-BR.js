// MFOP Specification v1.0 — conteúdo estruturado para página de revisão por pares
// Cada seção: { id, title, level (1=h2, 2=h3), content, blocks? }
// blocks: [{ type: 'table', headers: [...], rows: [[...]] } | { type: 'note', content }]

export const mfopMeta = {
  title: 'Protocolo de Federação e Orquestração Mahalaxmi',
  shortTitle: 'MFOP',
  version: '1.0',
  status: 'Rascunho para Revisão por Pares',
  date: 'Março de 2026',
...
O MFOP é projetado para operar em três configurações de implantação simultâneas: malhas empresariais privadas, nas quais os nós são de propriedade e operados por uma única organização; pools de nuvem gerenciados pelo provedor da plataforma; e marketplaces comunitários abertos, nos quais qualquer operador de nó pode contribuir com computação em troca de liquidação econômica. O protocolo é agnóstico em relação ao provedor de modelo de IA subjacente e foi projetado para evoluir junto com o cenário de segurança e conformidade de IA.`,
  },
  {
    ...
    title: '1. Introdução',
    level: 1,
    content: `O crescimento das implantações de modelos de linguagem de grande escala (LLM) em ambientes empresariais criou a necessidade de uma camada de coordenação capaz de abranger infraestrutura de computação heterogênea, ao mesmo tempo em que satisfaz requisitos de conformidade, faturamento e segurança que variam por jurisdição e setor.
...
Esta especificação define o protocolo de comunicação, os formatos de dados, os mecanismos criptográficos e os requisitos comportamentais para todos os componentes de uma federação MFOP em conformidade.`,
  ...
  {
    id: 'section-2',
    title: '2. Terminologia',
    level: 1,
    content: `As palavras-chave "DEVE", "NÃO DEVE", "OBRIGATÓRIO", "DEVERÁ", "NÃO DEVERÁ", "DEVERIA", "NÃO DEVERIA", "RECOMENDADO", "NÃO RECOMENDADO", "PODE" e "OPCIONAL" neste documento devem ser interpretadas conforme descrito no BCP 14 [RFC2119] [RFC8174].
...
**NeMo Guardrails** — O framework de segurança NVIDIA NeMo utilizado pelos nós MFOP para validação de políticas de segurança de IA e filtragem de saídas.`,
  ...
  {
    id: 'section-3',
    title: '3. Identidade e Registro de Nós',
    level: 1,
    content: `Cada nó em uma federação MFOP é identificado por um identificador de nó estável e globalmente único (node_id). O node_id é um UUID de 128 bits (versão 4) atribuído no momento do registro e persiste entre reinicializações do nó e atualizações de software.
...
Os nós DEVEM enviar um heartbeat para POST /v1/federation/nodes/{id}/heartbeat pelo menos uma vez a cada 60 segundos. Um nó que perde três janelas consecutivas de heartbeat é marcado como INATIVO e excluído do roteamento. Os nós podem cancelar o registro voluntariamente via DELETE /v1/federation/nodes/{id}.`,
  ...
  {
    id: 'section-4',
    title: '4. Anúncio de Capacidades',
    level: 1,
    content: `O anúncio de capacidades de um nó declara os modelos de IA disponíveis no nó, as características de hardware relevantes para o roteamento de tarefas e as certificações de conformidade detidas pelo operador do nó.
...
Os nós DEVEM atualizar seu anúncio de capacidades via PUT /v1/federation/nodes/{id}/capabilities sempre que seus modelos disponíveis ou configuração de hardware mudar. A federação propaga os anúncios de capacidades atualizados para a camada de roteamento em até 30 segundos.`,
  ...
  {
    id: 'section-5',
    title: '5. Roteamento de Tarefas Ciente de Zonas de Conformidade',
    level: 1,
    content: `O MFOP roteia cada tarefa para um nó que satisfaz os requisitos de zona de conformidade da tarefa. A satisfação da zona de conformidade é uma restrição rígida: uma tarefa NÃO DEVE ser roteada para um nó que não esteja certificado para a zona de conformidade da tarefa.
...
As regras de afinidade afetam apenas o componente pontuação_afinidade; a certificação de zona de conformidade e a capacidade permanecem como restrições rígidas.`,
  ...
  {
    id: 'section-6',
    title: '6. Particionamento Semântico de Entradas',
    level: 1,
    content: `Para tarefas cuja entrada excede o max_context_tokens de um único nó, o MFOP fornece um mecanismo de particionamento semântico que divide a entrada em sub-tarefas coerentes, roteia cada sub-tarefa de forma independente e agrega os resultados.
...
Assim que todas as sub-tarefas forem concluídas, a camada de agregação da federação monta os resultados na ordem do número de sequência. Para partições sliding_window, o agregador remove duplicatas de conteúdo nas regiões de sobreposição usando uma mesclagem de subsequência comum mais longa. O resultado montado é retornado ao submetente como um único JobResult com um array de sub_job_receipts.`,
  ...
  {
    id: 'section-7',
    title: '7. Recibos de Faturamento Assinados Criptograficamente',
    level: 1,
    content: `Cada execução de tarefa concluída produz um BillingReceipt assinado pelo nó executor. Os recibos assinados são o registro autoritativo para liquidação econômica e resolução de disputas.
...
A federação armazena todos os recibos por um mínimo de 7 anos para atender aos requisitos de auditoria de conformidade. Os submetentes podem recuperar seus recibos via GET /v1/federation/receipts. Os operadores de nós podem recuperar os recibos das tarefas que executaram via GET /v1/federation/nodes/{id}/receipts.`,
  ...
  {
    id: 'section-8',
    title: '8. Liquidação Econômica Configurável',
    level: 1,
    content: `O MFOP separa o faturamento (o acúmulo de recibos assinados) da liquidação (a transferência financeira de fundos). A liquidação é configurável e pode ocorrer em diferentes cronogramas para diferentes tipos de participantes.
...
Os operadores de nós recebem pagamentos via Stripe Connect ao final de cada período de liquidação, desde que seus ganhos acumulados excedam o limite minimum_payout_usd. Operadores que não atingem o limite acumulam seus ganhos para o próximo período.`,
  ...
  {
    id: 'section-9',
    title: '9. Modelo de Segurança',
    level: 1,
    content: `O MFOP implementa um modelo de segurança em três camadas: segurança de transporte, validação de políticas de segurança de IA e isolamento de sandbox de execução.
...
Todas as decisões de roteamento de tarefas, assinaturas de recibos e eventos de liquidação são gravados em um log de auditoria somente para acréscimo. O log de auditoria é encadeado criptograficamente usando hashes SHA-256 (cada entrada inclui o hash da entrada anterior). O log de auditoria não pode ser modificado; apenas operações de acréscimo são permitidas.`,
  ...
  {
    id: 'section-10',
    title: '10. Protocolo de Comunicação',
    level: 1,
    content: `O MFOP usa JSON sobre HTTPS para toda comunicação via API. Conexões WebSocket são suportadas para transmissão em streaming da saída de tarefas (veja a Seção 10.2).
...
As requisições de submissão de tarefas DEVERIAM incluir um cabeçalho Idempotency-Key (UUID). Se uma requisição com o mesmo Idempotency-Key for recebida dentro da janela de idempotência (24 horas), a federação retorna a resposta original sem re-executar a tarefa. Isso protege contra submissões duplicadas causadas por novas tentativas de rede.`,
  ...
  {
    id: 'appendix-a',
    title: 'Apêndice A. Referência da API REST',
    level: 1,
    content: `Este apêndice lista os endpoints da API REST do MFOP. Todos os endpoints requerem um cabeçalho X-Channel-API-Key, salvo indicação em contrário. Caminho base: /v1/federation`,
    blocks: [
      {
        type: 'table',
        headers: ['Método + Caminho', 'Nome', 'Descrição'],
        rows: [
  ...
  {
    id: 'appendix-b',
    title: 'Apêndice B. Requisitos de Políticas por Zona de Conformidade',
    level: 1,
    content: `Cada zona de conformidade requer capacidades específicas de política NeMo Guardrails além da linha de base. A tabela a seguir resume os rails mínimos obrigatórios por zona.`,
    blocks: [
      {
        type: 'table',
        headers: ['Zona', 'Rails Obrigatórios Além da Linha de Base'],
        rows: [
  ...
  },
  {
    ...
    level: 1,
    content: `A autora deseja reconhecer a equipe NVIDIA NeMo pelas plataformas NeMo Guardrails e NemoClaw OpenShell, que fornecem a infraestrutura de segurança fundamental referenciada nesta especificação. O modelo de segurança do MFOP é projetado para evoluir junto com essas plataformas à medida que amadurecem.
