...
/// Go-specific extractor with capitalization-based visibility.
...
impl GoExtractor {
    ...
    pub fn new(registry: &LanguageRegistry, i18n: &I18nService) -> MahalaxmiResult<Self> {
        let config = registry.get(&SupportedLanguage::Go).ok_or_else(|| {
        ...
        let inner = TreeSitterExtractor::new(SupportedLanguage::Go, config, i18n)?;
        Ok(Self { inner })
    }
}
...
impl SymbolExtractor for GoExtractor {
    fn extract_symbols(
        ...
        // Go: capitalized names are exported (public)
        for symbol in &mut symbols {
            if symbol.name.starts_with(|c: char| c.is_uppercase()) {
