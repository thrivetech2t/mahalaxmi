...
/// C-specific extractor with static-based visibility.
...
impl CExtractor {
    ...
    pub fn new(registry: &LanguageRegistry, i18n: &I18nService) -> MahalaxmiResult<Self> {
        let config = registry.get(&SupportedLanguage::C).ok_or_else(|| {
        ...
        let inner = TreeSitterExtractor::new(SupportedLanguage::C, config, i18n)?;
        Ok(Self { inner })
    }
}
...
impl SymbolExtractor for CExtractor {
    fn extract_symbols(
        ...
        // C: static = private, else public
        for symbol in &mut symbols {
            let line_text = source
                .lines()
                .nth(symbol.line_start.saturating_sub(1))
                .unwrap_or("");
            if line_text.trim_start().starts_with("static ") {
